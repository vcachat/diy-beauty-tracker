...
function TreatmentsTab() {
  ...
  const handleTreatImg = async (e) => {
    ...
    for (const file of files) {
      ...
      setForm(f => ({ ...f, images: [...(f.images || []), { data, name: file.name }] }));
    }
  ...
  const save = async () => {
    if (!form.type) return;
    const id = Date.now();
    // Save treatment images to treatment_images table
    const imgRows = (form.images || []).map(img => ({
    ...
    if (imgRows.length > 0) await supabase.from("treatment_images").insert(imgRows);
                ...
                <div style={S.treatImgRow}>
                  {form.images.map((img, i) => (
                    <div key={i} style={{ position: "relative", display: "inline-block" }}>
                      <img src={img.data} alt="" style={S.treatThumb} />
...
function ProgressTab() {
  ...
  const [form, setForm] = useState({ date: today(), label: "", notes: "", imageData: null });
  ...
  const save = async () => {
    ...
    await supabase.from("progress_photos").insert({
      ...
      notes: form.notes, image_data: form.imageData
    });
    setForm({ date: today(), label: "", notes: "", imageData: null });
    setShowForm(false);
            ...
            {photos.map(p => {
              ...
              return (
                <div key={p.id} style={{ ...S.photoCard, outline: sel ? "2.5px solid #7aadcf" : "none", cursor: "pointer" }}
                  ...
                  <img src={p.image_data} alt="" style={S.photoImg} />
            ...
            <div style={S.compareView}>
              {compare.map(id => { const p = photos.find(x => x.id === id); return p ? (
                <div key={id} style={S.compareItem}>
                  <img src={p.image_data} alt="" style={{ width: "100%", borderRadius: 12, objectFit: "cover", maxHeight: 260 }} />
        ...
        <div style={S.photoGrid}>
          {photos.map(p => (
            <div key={p.id} style={S.photoCard}>
              <img src={p.image_data} alt={p.label || "progress"} style={S.photoImg} />
      ...
      {showForm && (
        <Modal title="Add Progress Photo" onClose={() => setShowForm(false)}>
          <div style={S.form}>
            ...
            {form.imageData && <img src={form.imageData} alt="preview" style={{ width: "100%", borderRadius: 12, maxHeight: 200, objectFit: "cover" }} />}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Field label="Date"><input style={S.input} type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} /></Field>
          ...
          <div style={S.formActions}>
            <button style={S.cancelBtn} onClick={() => setShowForm(false)}>Cancel</button>
            <button style={{ ...S.primaryBtn, opacity: form.imageData ? 1 : 0.45 }} onClick={save}>Save Photo</button>
          </div>
        </Modal>
      )}
    </div>
...
function GuidesTab() {
  ...
  const [form, setForm] = useState({ title: "", category: "", description: "", fileData: null, fileName: "", fileType: "", addedDate: today() });
  const fileRef = useRef();
  ...
  const handleFile = async (e) => {
    ...
    if (!file) return;
    const data = await toBase64(file);
    setForm(f => ({ ...f, fileData: data, fileName: file.name, fileType: file.type }));
  };
  ...
  const save = async () => {
    ...
    await supabase.from("guides").insert({
      ...
      file_type: form.fileType, file_data: form.fileData,
      added_date: form.addedDate
    });
    setForm({ title: "", category: "", description: "", fileData: null, fileName: "", fileType: "", addedDate: today() });
    setShowForm(false);
    if (fileRef.current) fileRef.current.value = "";
    load();
  };
  ...
  const download = (g) => {
    if (!g.file_data) return;
    const a = document.createElement("a"); a.href = g.file_data; a.download = g.file_name || g.title; a.click();
  };
        ...
        {filtered.map(g => (
          <div key={g.id} style={S.guideCard}>
            <div style={{ ...S.guidePreviewWrap, background: isPDF(g) ? "#fff5f0" : isImage(g) ? "#f7f5f2" : "#f5f0ff", cursor: "pointer" }}
              ...
              {isImage(g) && g.file_data
                ? <img src={g.file_data} alt={g.title} style={S.guidePreviewImg} />
            ...
            <div style={S.guideInfo}>
              ...
              <div style={S.guideFooter}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {g.file_data && <button style={S.ghostBtn} onClick={() => download(g)}>↓ Save</button>}
        ...
        <div style={S.overlay} onClick={() => setViewGuide(null)}>
          <div style={{ ...S.modal, maxHeight: "95vh", borderRadius: 16 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ overflowY: "auto", flex: 1, paddingBottom: 20 }}>
              {isImage(viewGuide) && viewGuide.file_data && <img src={viewGuide.file_data} alt="" style={{ width: "100%", borderRadius: 10 }} />}
              {isPDF(viewGuide) && viewGuide.file_data && <iframe src={viewGuide.file_data} title={viewGuide.title} style={{ width: "100%", height: 500, border: "none", borderRadius: 10 }} />}
        ...
        <Modal title="Add to Library" onClose={() => setShowForm(false)}>
          <div style={S.form}>
            ...
            <Field label="File or Image">
              ...
              {form.fileData && isImage(form) && <img src={form.fileData} alt="preview" style={{ width: "100%", borderRadius: 10, maxHeight: 160, objectFit: "cover", marginTop: 6 }} />}
              {form.fileName && !isImage(form) && <div style={{ fontSize: 13, color: "#c4845a", marginTop: 4 }}>📎 {form.fileName}</div>}
            </Field>
