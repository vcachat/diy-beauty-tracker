...
const GUIDE_CATEGORIES = ["Protocol", "Technique", "Aftercare", "Mapping", "Product Guide", "
// ─── Helpers ─────────────────────────────────────────────────────────────────
...
setLoading(true);
const { data } = await supabase.from("products").select("*").order("id", { ascending: fal
setProducts(data || []);
setLoading(false);
}, []);
useEffect(() => { load(); }, [load]);
...
setForm(f => ({ ...f, concern: f.concern.includes(c) ? f.concern.filter(x => x !== c) : [
...
<div style={S.summaryItem}><span style={S.summaryNum}>{products.length}</span><span s
<div style={S.summaryDivider} />
<div style={S.summaryItem}><span style={S.summaryNum}>{products.filter(p => !p.finish
<div style={S.summaryDivider} />
<div style={S.summaryItem}><span style={S.summaryNum}>${total.toFixed(0)}</span><span
</div>
<div style={S.rowBetween}>
<div style={S.filterRow}>
{["All", "Active", "Finished"].map(f => (
<button key={f} style={{ ...S.chip, ...(filter === f ? S.chipActive : {}) }} onCl
))}
</div>
</div>
...
<div style={S.empty}><div style={S.emptyIcon}> </div><p style={S.emptyText}>No produ
)}
...
<div style={S.tagWrap}>
{p.concern.map(c => (
<span key={c} style={{ ...S.concernTag, background: CONCERN_COLORS[c] || "#
))}
</div>
)}
<div style={S.cardActions}>
{!p.finished && <button style={S.editBtn} onClick={() => openEdit(p)}>✎ Edit</b
...
{showForm && (
<Modal title={editingId ? "Edit Product" : "Add Product"} onClose={() => { setShowFor
<div style={S.form}>
<Field label="Product Name *"><input style={S.input} value={form.name} onChange={
<Field label="Brand"><input style={S.input} value={form.brand} onChange={e => set
<Field label="Category">
<select style={S.input} value={form.category} onChange={e => setForm(f => ({ ..
<option value="">Select...</option>
{PRODUCT_CATEGORIES.map(c => <option key={c}>{c}</option>)}
</select>
</Field>
<Field label="Purpose / How you use it"><input style={S.input} value={form.purpos
<Field label="Skin Concerns">
<div style={S.checkGrid}>
{SKIN_CONCERNS.map(c => (
<label key={c} style={S.checkLabel}>
<input type="checkbox" checked={form.concern.includes(c)} onChange={() =>
{c}
</label>
))}
</div>
...
<Field label="Purchase Date"><input style={S.input} type="date" value={form.pur
<Field label="Price ($)"><input style={S.input} type="number" value={form.price
</div>
<Field label="Purchased From"><input style={S.input} value={form.purchasedFrom} o
<Field label="Notes"><textarea style={{ ...S.input, height: 70, resize: "vertical
</div>
<div style={S.formActions}>
<button style={S.cancelBtn} onClick={() => { setShowForm(false); setEditingId(nul
<button style={S.primaryBtn} onClick={save}>{editingId ? "Save Changes" : "Save P
</div>
</Modal>
)}
</div>
...
const [form, setForm] = useState({ type: "", date: today(), products: [], duration: "", rat
const [schedForm, setSchedForm] = useState({ type: "", frequency: "", lastDone: today(), no
...
useEffect(() => { load(); }, [load]);
const toggleProd = (id) =>
setForm(f => ({ ...f, products: f.products.includes(id) ? f.products.filter(x => x !== id
...
setForm({ type: "", date: today(), products: [], duration: "", rating: 5, notes: "", next
setShowForm(false);
load();
};
...
await supabase.from("schedules").update({ last_done: today(), next_due: nextDue }).eq("id
load();
};
...
const typeCount = treatments.reduce((acc, t) => { acc[t.type] = (acc[t.type] || 0) + const topTypes = Object.entries(typeCount).sort((a, b) => b[1] - a[1]).slice(0, 4);
...
<button style={{ ...S.toggleBtn, ...(view === "log" ? S.toggleActive : {}) }} onCli
<button style={{ ...S.toggleBtn, ...(view === "schedules" ? S.toggleActive : Recurring {overdueCount > 0 && <span style={S.overduebadge}>{overdueCount}</span>
</button>
</div>
<button style={S.primaryBtn} onClick={() => view === "log" ? setShowForm(true) {view === "log" ? "+ Log" : "+ Schedule"}
</button>
</div>
: setS
{loading && <Spinner />}
{view === "log" && !loading && (
<>
{treatments.length === 0 && <div style={S.empty}><div style={S.emptyIcon}> </div><
...
{t.next_session && <div style={{ ...S.treatMeta, color: "#e8847a" }}> {
</div>
<div style={S.treatRight}>
<div style={S.stars}>{"★".repeat(t.rating || 0)}{"☆".repeat(5 - (t.rating
{usedProds.length > 0 && (
<div style={S.tagWrap}>{usedProds.map(p => <span key={p.id} style={S.pr
)}
...
<>
{schedules.length === 0 && <div style={S.empty}><div style={S.emptyIcon}> </div><p
<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
{schedules.map(s => {
const days = daysUntil(s.next_due);
const overdue = days < 0, dueSoon = days >= 0 && days <= 7;
return (
<div key={s.id} style={{ ...S.schedCard, ...(overdue ? S.schedOverdue : dueSo
<div style={S.schedTop}>
<div>
<div style={S.schedType}>{s.type}</div>
<div style={S.schedFreq}>{s.frequency}</div>
</div>
<div style={{ ...S.schedBadge, background: overdue ? "#fde8e8" : dueSoon
{overdue ? `${Math.abs(days)}d overdue` : days === 0 ? "Due today!" : `
</div>
</div>
...
<button style={S.primaryBtn} onClick={() => markDone(s)}>✓ Done Today</bu
<button style={S.dangerBtn} onClick={() => removeSchedule(s.id)}>Remove</
</div>
</div>
);
})}
...
<select style={S.input} value={form.type} onChange={e => setForm(f => ({ <option value="">Select treatment...</option>
{TREATMENT_TYPES.map(t => <option key={t}>{t}</option>)}
</select>
</Field>
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
<Field label="Date"><input style={S.input} type="date" value={form.date} onChan
<Field label="Duration (min)"><input style={S.input} type="number" value={form.
</div>
...
<input type="checkbox" checked={form.products.includes(p.id)} onChange=
{p.name}
</label>
))}
</div>}
...
{[1, 2, 3, 4, 5].map(n => (
<button key={n} style={{ ...S.starBtn, color: form.rating >= n ? "#f5a623"
onClick={() => setForm(f => ({ ...f, rating: n }))}>★</button>
))}
</div>
</Field>
<Field label="Schedule Next Session"><input style={S.input} type="date" value={fo
<Field label="Notes / Results"><textarea style={{ ...S.input, height: 80, resize:
<Field label="Treatment Area Photos">
<input ref={treatImgRef} type="file" accept="image/*" multiple style={{ fontSiz
{form.images?.length > 0 && (
<div style={S.treatImgRow}>
{form.images.map((img, i) => (
<div key={i} style={{ position: "relative", display: "inline-block" }}>
<img src={img.previewUrl} alt="" style={S.treatThumb} />
<button style={S.imgRemoveBtn} onClick={() => setForm(f => ({ ...f, ima
</div>
))}
</div>
)}
...
<div style={S.form}>
<Field label="Treatment Type *">
<select style={S.input} value={schedForm.type} onChange={e => setSchedForm(f =>
...
<input style={S.input} type="date" value={schedForm.lastDone} onChange={e => se
</Field>
<Field label="Notes">
<input style={S.input} value={schedForm.notes} onChange={e => setSchedForm(f =>
</Field>
{schedForm.type && schedForm.frequency && (
<div style={S.schedPreview}>
<span style={{ fontSize: 13, color: "#5a8a5a" }}>
✓ Next due: <strong>{formatDate(addDays(schedForm.lastDone, RECUR_OPTIONS.f
</span>
</div>
)}
</div>
<div style={S.formActions}>
<button style={S.cancelBtn} onClick={() => setShowScheduleForm(false)}>Cancel</bu
...
return (
<div key={k} style={{ ...S.calCell, ...(isToday ? S.calToday : {}), ...(ts.length
...
const [form, setForm] = useState({ date: today(), label: "", notes: "", previewFile: null,
const fileRef = useRef();
const load = useCallback(async () => {
setLoading(true);
const { data } = await supabase.from("progress_photos").select("*").order("id", { ascendi
setPhotos(data || []);
setLoading(false);
}, []);
useEffect(() => { load(); }, [load]);
...
setForm({ date: today(), label: "", notes: "", previewFile: null, previewUrl: null setShowForm(false);
...
<span style={{ fontSize: 13, color: "#5a7a9a" }}> Compare two photos side by side
...
<div style={S.empty}><div style={S.emptyIcon}> )}
</div><p style={S.emptyText}>Add prog
{compare !== null ? (
<div>
<p style={{ fontSize: 13, color: "#8a9aaa", marginBottom: 10 }}>Select two photos t
<div style={S.photoGrid}>
{photos.map(p => {
const sel = compare.includes(p.id);
return (
<div key={p.id} style={{ ...S.photoCard, outline: sel ? "2.5px solid #7aadcf"
onClick={() => { if (sel) setCompare(compare.filter(x => x !== p.id)); else
<img src={p.image_url} alt="" style={S.photoImg} />
<div style={S.photoInfo}><div style={S.photoDate}>{formatDate(p.date)}</div
...
<img src={p.image_url} alt="" style={{ width: "100%", borderRadius: 12, obj
...
<div style={S.form}>
<Field label="Photo *">
<input ref={fileRef} type="file" accept="image/*" style={{ fontSize: 13, </Field>
{form.previewUrl && <img src={form.previewUrl} alt="preview" style={{ width: "100
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
<Field label="Date"><input style={S.input} type="date" value={form.date} <Field label="Label"><input style={S.input} value={form.label} onChange={e => s
</div>
<Field label="Notes"><textarea style={{ ...S.input, height: 70, resize: "vertical
</div>
<div style={S.formActions}>
<button style={S.cancelBtn} onClick={() => setShowForm(false)}>Cancel</button>
<button style={{ ...S.primaryBtn, opacity: form.previewFile ? 1 : 0.45 }} onClick
</div>
</Modal>
)}
</div>
...
const [form, setForm] = useState({ title: "", category: "", description: "", previewFile: n
const fileRef = useRef();
const load = useCallback(async () => {
setLoading(true);
...
const file = e.target.files[0];
if (!file) return;
setForm(f => ({ ...f, previewFile: file, fileName: file.name, fileType: file.type, previe
};
const save = async () => {
...
setForm({ title: "", category: "", description: "", previewFile: null, fileName: "", file
setShowForm(false);
if (fileRef.current) fileRef.current.value = "";
load();
};
...
<div style={S.guidesHeroSub}>Save protocols, technique guides, treatment maps & ref
</div>
</div>
<div style={S.rowBetween}>
<div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
<button style={{ ...S.chip, ...(filterCat === "All" ? S.chipActive : {}) }} onClick
{usedCats.map(c => <button key={c} style={{ ...S.chip, ...(filterCat === c ? </div>
<button style={S.primaryBtn} onClick={() => setShowForm(true)}>+ Add File</button>
</div>
S.chip
{loading && <Spinner />}
{!loading && filtered.length === 0 && (
<div style={S.empty}><div style={S.emptyIcon}> </div><p style={S.emptyText}>Save you
)}
<div style={S.guidesGrid}>
{filtered.map(g => (
<div key={g.id} style={S.guideCard}>
<div style={{ ...S.guidePreviewWrap, background: isPDF(g) ? "#fff5f0" : isImage(g
onClick={() => setViewGuide(g)}>
{isImage(g) && g.file_url
? <img src={g.file_url} alt={g.title} style={S.guidePreviewImg} />
: <><div style={S.guideFileIcon}>{isPDF(g) ? " " : " "}</div><div style={S
}
</div>
...
{g.file_url && <button style={S.ghostBtn} onClick={() => download(g)}>↓ Sav
...
<div style={{ ...S.modal, maxHeight: "95vh", borderRadius: 16 }} onClick={e => e.st
<div style={S.modalHeader}>
<span style={S.modalTitle}>{viewGuide.title}</span>
<button style={S.closeBtn} onClick={() => setViewGuide(null)}>×</button>
</div>
<div style={{ overflowY: "auto", flex: 1, paddingBottom: 20 }}>
{isImage(viewGuide) && viewGuide.file_url && <img src={viewGuide.file_url} alt=
{isPDF(viewGuide) && viewGuide.file_url && <iframe src={viewGuide.file_url} tit
{viewGuide.description && <p style={{ fontSize: 14, color: "#5a5050", marginTop
</div>
</div>
</div>
)}
...
<div style={S.form}>
<Field label="Title *"><input style={S.input} value={form.title} onChange={e => s
<Field label="Category">
<select style={S.input} value={form.category} onChange={e => setForm(f => ({ ..
<option value="">Select...</option>
...
<input ref={fileRef} type="file" accept="image/*,.pdf,.doc,.docx,.txt" style={{
{form.previewUrl && form.fileType?.startsWith("image/") && <img src={form.previ
{form.fileName && !form.fileType?.startsWith("image/") && <div style={{ fontSiz
</Field>
<Field label="Description / Notes"><textarea style={{ ...S.input, height: 80, res
</div>
<div style={S.formActions}>
<button style={S.cancelBtn} onClick={() => setShowForm(false)}>Cancel</button>
...
<div style={S.nav}>
{TABS.map((t, i) => (
<button key={t} style={{ ...S.navBtn, ...(tab === i ? S.navActive : {}) }} onClick=
...
// ─── Styles ───────────────────────────────────────────────────────────────────
const S = {
app: { minHeight: "100vh", background: "#f7f5f2", fontFamily: "'DM Sans', sans-serif", colo
...
appName: { fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 500, color: "
appSub: { fontSize: 12, color: "#b0a898", marginTop: 2, letterSpacing: "0.04em" },
nav: { display: "flex", background: "#fff", borderBottom: "1px solid #ede8e0", position: "s
navBtn: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding:
...
summaryBar: { display: "flex", background: "#fff", borderRadius: 14, padding: "14px 20px",
summaryItem: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2 },
summaryNum: { fontSize: 22, fontWeight: 600, color: "#c4845a", fontFamily: "'Playfair Displ
summaryLbl: { fontSize: 11, color: "#b0a898", textTransform: "uppercase", letterSpacing: "0
summaryDivider: { width: 1, height: 32, background: "#ede8e0" },
rowBetween: { display: "flex", alignItems: "center", justifyContent: "space-between", margi
sectionTitle: { fontSize: 13, color: "#8a8078", fontWeight: 500, textTransform: "uppercase"
filterRow: { display: "flex", gap: 6 },
chip: { padding: "5px 13px", borderRadius: 20, border: "1px solid #e0d8ce", background: "#f
chipActive: { background: "#c4845a", borderColor: "#c4845a", color: "#fff" },
primaryBtn: { padding: "8px 18px", background: "#c4845a", color: "#fff", border: "none", bo
ghostBtn: { flex: 1, padding: "6px 0", border: "1px solid #e0d8ce", background: "transparen
gap: 3
dangerBtn: { padding: "6px 10px", border: "1px solid #f0dada", background: "transparent", c
editBtn: { flex: 1, padding: "6px 0", border: "1px solid #d8ecd8", background: "#f5fbf5", c
cancelBtn: { flex: 1, padding: "11px", border: "1px solid #e0d8ce", background: "#fff", bor
xBtn: { position: "absolute", top: 8, right: 8, background: "none", border: "none", color:
...
card: { background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #ede8e0", box
cardGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))",
cardTopRow: { display: "flex", justifyContent: "space-between", marginBottom: 8, alignItems
categoryBadge: { background: "#f5f0ea", color: "#9a8878", fontSize: 10, padding: "3px 9px",
priceBadge: { background: "#fdf3ea", color: "#c4845a", fontSize: 11, fontWeight: 600, paddi
cardName: { fontSize: 15, fontWeight: 600, color: "#2c2820", marginBottom: 2 },
cardBrand: { fontSize: 12, color: "#b0a898", marginBottom: 5 },
cardPurposeBlock: { background: "#fdf8f4", borderLeft: "2px solid #e8c9b0", borderRadius: "
cardPurposeLabel: { fontSize: 9, color: "#c4845a", textTransform: "uppercase", letterSpacin
cardPurpose: { fontSize: 12, color: "#5a5050", lineHeight: 1.4 },
cardDate: { fontSize: 11, color: "#c0b8b0", marginTop: 6, marginBottom: 8 },
cardMeta: { marginTop: 6, marginBottom: 8, display: "flex", flexDirection: "column", cardMetaRow: { fontSize: 11, color: "#b0a898" },
...
prodTag: { background: "#f0ece6", color: "#7a7068", fontSize: 10, padding: "2px 8px", borde
statsRow: { display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 },
statCard: { background: "#fff", border: "1px solid #ede8e0", borderRadius: 12, padding: "10
statNum: { fontSize: 20, fontWeight: 700, color: "#c4845a", fontFamily: "'Playfair Display'
statLbl: { fontSize: 10, color: "#b0a898", textAlign: "center", marginTop: 2 },
viewToggleRow: { display: "flex", alignItems: "center", justifyContent: "space-between", ma
viewToggle: { display: "flex", background: "#f0ece6", borderRadius: 20, padding: 3, gap: 2
toggleBtn: { padding: "6px 14px", border: "none", borderRadius: 17, background: "transparen
toggleActive: { background: "#fff", color: "#2c2820", boxShadow: "0 1px 4px rgba(0,0,0,0.08
overduebadge: { background: "#e85050", color: "#fff", borderRadius: "50%", width: 16, heigh
treatCard: { background: "#fff", borderRadius: 14, padding: "14px 14px 14px 16px", border:
...
treatThumb: { width: 56, height: 56, borderRadius: 8, objectFit: "cover", border: "1px soli
imgRemoveBtn: { position: "absolute", top: -6, right: -6, width: 18, height: 18, borderRadi
schedCard: { background: "#fff", borderRadius: 14, padding: 14, border: "1px solid #ede8e0"
schedOverdue: { border: "1.5px solid #f0c8c8", background: "#fffafa" },
schedSoon: { border: "1.5px solid #f0e0b8", background: "#fffdf5" },
schedTop: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", mar
schedType: { fontSize: 15, fontWeight: 600, color: "#2c2820" },
schedFreq: { fontSize: 12, color: "#b0a898", marginTop: 2 },
schedBadge: { fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 12, whiteSp
...
schedPreview: { background: "#f0fbf0", border: "1px solid #c8e8c8", borderRadius: 10, paddi
freqGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 },
freqBtn: { padding: "8px 10px", border: "1px solid #e0d8ce", borderRadius: 10, background:
freqActive: { background: "#c4845a", borderColor: "#c4845a", color: "#fff", fontWeight: 600
calHeader: { display: "flex", alignItems: "center", justifyContent: "space-between", margin
calNav: { width: 34, height: 34, background: "#fff", border: "1px solid #e0d8ce", borderRad
...
calDayName: { textAlign: "center", fontSize: 10, color: "#c0b8b0", padding: "4px 0 6px", fo
calEmpty: { minHeight: 52 },
calCell: { minHeight: 52, background: "#fff", borderRadius: 8, padding: "5px 4px", border:
...
upcomingRow: { background: "#fff", borderRadius: 10, padding: "10px 14px", border: "1px sol
upcomingDate: { fontSize: 12, color: "#b0a898", minWidth: 70 },
upcomingType: { fontSize: 13, fontWeight: 600, color: "#2c2820", flex: 1 },
photoGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))",
photoCard: { background: "#fff", borderRadius: 14, overflow: "hidden", border: "1px solid #
...
selectedBadge: { position: "absolute", top: 6, right: 6, background: "#c4845a", color: "#ff
compareBanner: { background: "#edf4fa", border: "1px solid #cce0f0", borderRadius: 12, padd
compareBtn: { padding: "5px 14px", background: "#e8f2fa", border: "1px solid #bdd8ec", bord
compareView: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 16, bac
compareItem: { display: "flex", flexDirection: "column", gap: 5 },
guidesHero: { display: "flex", alignItems: "center", gap: 14, background: "linear-gradient(
guidesHeroIcon: { fontSize: 32 },
guidesHeroTitle: { fontFamily: "'Playfair Display', serif", fontSize: 16, color: "#2c2820",
guidesHeroSub: { fontSize: 12, color: "#b0a898", lineHeight: 1.4 },
guidesGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))"
guideCard: { background: "#fff", borderRadius: 16, border: "1px solid #ede8e0", overflow: "
guidePreviewWrap: { height: 120, display: "flex", flexDirection: "column", alignItems: "cen
guidePreviewImg: { width: "100%", height: "100%", objectFit: "cover", display: "block" },
guideFileIcon: { fontSize: 36, marginBottom: 6 },
guideFileName: { fontSize: 11, color: "#b0a898", textAlign: "center", padding: "0 8px", wor
guideInfo: { padding: "12px 12px 10px" },
guideTitle: { fontSize: 14, fontWeight: 600, color: "#2c2820", marginBottom: 5 },
guideCatBadge: { display: "inline-block", background: "#fdf0e8", color: "#c4845a", fontSize
guideDesc: { fontSize: 12, color: "#8a8078", lineHeight: 1.5, marginBottom: 6 },
guideFooter: { display: "flex", alignItems: "center", justifyContent: "space-between", marg
guideDate: { fontSize: 11, color: "#c0b8b0" },
overlay: { position: "fixed", inset: 0, background: "rgba(30,25,20,0.4)", display: "flex",
modal: { background: "#fff", borderRadius: "20px 20px 0 0", padding: "20px 20px 0", width:
modalHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", marg
modalTitle: { fontFamily: "'Playfair Display', serif", fontSize: 18, color: "#2c2820" },
closeBtn: { background: "none", border: "none", fontSize: 26, color: "#c0b8b0", cursor: "po
form: { display: "flex", flexDirection: "column", gap: 12, paddingBottom: 8 },
label: { fontSize: 12, color: "#b0a898", fontWeight: 500, textTransform: "uppercase", lette
input: { padding: "9px 12px", borderRadius: 10, border: "1px solid #e0d8ce", fontSize: 14,
checkGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 },
checkLabel: { display: "flex", alignItems: "center", gap: 7, fontSize: 13, color: "#5a5050"
